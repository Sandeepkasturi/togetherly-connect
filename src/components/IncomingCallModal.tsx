...
const IncomingCallModal = ({ call, caller, onAccept, onDecline }: IncomingCallModalProps) => {
    const isRoomInvite = call?.caller_peer_id?.startsWith('room_invite:');
    const roomTypeMatch = isRoomInvite ? call.caller_peer_id.match(/room_invite:([^:]+):(.+)/) : null;
    const roomType = roomTypeMatch ? roomTypeMatch[2] : null;
                        ...
                        <div className="absolute inset-0 rounded-[28px] animate-pulse"
                            style={{
                                background: isRoomInvite
                                    ? 'radial-gradient(circle at 50% 0%, rgba(191,90,242,0.15) 0%, transparent 70%)'
                                    : call.type === 'video'
                        ...
                        <div className="relative px-6 py-6 flex flex-col items-center gap-5">
                            ...
                            <div className="absolute top-5 right-5 flex items-center gap-1.5 px-2.5 py-1 rounded-full"
                                style={{
                                    background: isRoomInvite
                                        ...
                                        : call.type === 'video' ? 'rgba(10,132,255,0.18)' : 'rgba(48,209,88,0.15)'
                                }}>
                                {isRoomInvite ? <Video className="h-3.5 w-3.5 text-[#BF5AF2]" />
                                    : call.type === 'video'
                                ...
                                <span className="text-[11px] font-semibold"
                                    style={{
                                        color: isRoomInvite
                                            ? '#BF5AF2'
                                            : call.type === 'video' ? '#0A84FF' : '#30D158'
                                    }}>
                                    {isRoomInvite
                                        ? (roomType === 'party' ? 'Watch togetherly Invite' : 'Conference Invite')
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-[13px] text-white/50 mt-0.5">
                                    {isRoomInvite
                                        ? `Inviting you to a ${roomType === 'party' ? 'Watch togetherly' : 'Conference'} room...`
